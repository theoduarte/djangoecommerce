...
class CartItemManager(models.Manager):
    ...
    def add_item(self, cart_key, product):
        #cart_item, created = self.get_or_created(cart_key=cart_key, product=product, price=product.price)

        if self.filter(cart_key=cart_key, product=product).exists():
            created = False
            cart_item = self.get(cart_key=cart_key, product)
...
class CartItem(models.Model):
    ...
    class Meta:
        ...
        verbose_name_plural = 'Itens dos Carrrinhos'
